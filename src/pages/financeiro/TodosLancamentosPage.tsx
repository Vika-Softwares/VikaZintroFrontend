...
export const TodosLancamentosPage = (): JSX.Element => {
  ...
  const [filtroTipo, setFiltroTipo] = useState<"Todos" | "Pagar" | "Receber">(
    "Todos"
  );
  const [filtroStatus, setFiltroStatus] = useState<
    "Todos" | "Pendente" | "Pago" | "Recebido" | "Vencido"
  >("Todos");
  ...
  const allFilteredLancamentos = lancamentos.filter((lancamento) => {
    ...
    // Filtro por status
    const matchStatus =
      filtroStatus === "Todos" || lancamento.status === filtroStatus;
  ...
  const filteredLancamentos = allFilteredLancamentos.slice(
    startIndex,
    endIndex
  );
      ...
      <div className="flex flex-col gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Visão consolidada de contas a pagar e receber - {getNomeDoMes()}{" "}
            {anoAtual}
          </p>
        </div>
      </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
            <div>
              ...
              <p className="text-sm text-gray-500">
                {totalFiltered} lançamentos encontrados
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500">
                {searchTerm
                  ...
                  : `Não há lançamentos para ${getNomeDoMes()} ${anoAtual}`}
              </p>
            </div>
          )}
        </div>
